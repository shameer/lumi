...
names(tempDataList) <- c(names(normDataList), 'VSN.pairwise')
...
par(oldpar)
@
\caption{Comparison of the correlation between technical replicate chips after preprocessing. The VSN-pairwise method performed the VSN within each pair of technical replicate samples and then calculated their correlations.}
\label{fig:chipCor}
\end{figure} 
...
\Sexpr{sampleNames(x.lumi)[selChip[2]]}. The VSN-pairwise method performed the VSN only within the pair of technical replicate samples.}
\label{fig:techRep}
\end{figure} 
...
matplot(topNumList, (100 * highCorrNumMatrix/(topNumList %*% t(rep(1,ncol(highCorrNumMatrix))))),
 	type='l', xlab='Number of top probes by ranking their p-values (F-test)', 
...
matplot(topNumList, (100 * highCorrNumMatrix/(topNumList %*% t(rep(1,ncol(highCorrNumMatrix))))),
 	type='l', xlab='Number of top probes by ranking their p-values', 
	ylab='Percentage of concordant probes (%)', 
