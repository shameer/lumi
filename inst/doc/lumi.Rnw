...
Because all the Illumina microarrays use 50-mers, by using the nuID universal identifier, we are able to build one annotation database for different versions of the human (or other species) chips. Moreover, the nuID can be directly converted to the probe sequence, and used to get the most updated refSeq matches and annotations. Currently, annotation packages for human (\Rpackage{lumiHumanV1}) and mouse (\Rpackage{Mouse.lumi}) have been created. Packages for more species will be created in the future.
...
For Illumina microarray data, there are two ways to transfer the TargetID (or Probe Id) as nuID. The first way is to use the annotation file for the specific chip which is provided by Illumina company. \verb+'Human_RefSeq-8.csv'+ is an example annotation file for Human RefSeq-8 microarray. \Rfunction{addNuId2lumi} implements the function.
\begin{Sinput}
...
An alternative way is to load the Annotation library and match the targetID (or Probe Id) with nuID. The function can automatically check whether targetID or Probe Id  was used in the first column of the text data file, and convert them as nuID.
<<quiet=T>>=
...
> example.lumi <- addNuId2lumi(example.lumi, lib='lumiHumanV1')
