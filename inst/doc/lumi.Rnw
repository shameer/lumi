...
In order to install the lumi package, the user need to first install R and some R packages: \Rpackage{Biobase}, \Rpackage{affy}, \Rpackage{mgcv}, \Rpackage{annotate}, \Rpackage{methods}. If the user is also interested in using vsn method, then \Rpackage{vsn} package needs to be installed.
...
We strongly suggest outputting the header information when using BeadStudio, as shown in Figure \ref{fig:dataFileSample}. 

If a lumi annotation library is provided, the \Rfunction{lumiR} function will automatically mapping the ProbeId or TargetID as nuID (see annotation section for more details), and keep the mapping information in the featureData of the {\bf LumiBatch} object.
...
The quality control of a {\bf LumiBatch} object includes a data summary (the mean and standard deviation, sample correlation,  detectable probe ratio of each sample (microarray)) and different quality control plots.
...
% box plot
Plot the box plot of the {\bf LumiBatch} object. See Figure \ref{fig:box}.
\begin{Sinput}
> ## plot the box plot
...
Plot the pairwise sample correlation of the {\bf LumiBatch} object. See Figure \ref{fig:pairs}.
\begin{Sinput}
> ## plot the pair plot
...
% MA plot
The MA plot of the {\bf LumiBatch} object. See Figure \ref{fig:MAplot}.
\begin{Sinput}
> ## plot the MAplot
...
\centering
<<fig=true, width=10, height=10, quiet=T, echo=F>>=
plot(example.lumi, what='MAplot')	## pairwise MAplot
@
...
plot(example.lumi, what='cv')	## density plot of coefficient of varience
@
...
Plot the sampleRelation using MDS, see Figure \ref{fig:sampleRelation2}. The color of the sample is based on the sample type, which is \verb+"01", "02", "01", "02"+ for the sample data. Please see the help of \Rfunction{plotSampleRelation} and \Rfunction{plot-methods} for more details.
\begin{Sinput}
...
The \Rpackage{lumi} package provides \Rfunction{lumiB} function for background correction. As both \Rfunction{vst} and \Rfunction{log2} transforms require the expression value to be positive. The default background correction method ('forcePositive') will force all expression values to be positive by adding an offset (minus minimum value plus one). It does nothing if all expression values are positive. Other options of \Rfunction{lumiB} include 'none' and 'bg.adjust'. 'bg.adjust' method is based on the \Rfunction{bg.adjust} function in \Rpackage{affy} package. User can also provide their own background correction function with a LumiBatch Object as the first argument and return a LumiBatch Object with background corrected. See \Rfunction{lumiB} help document for more details.
...
Users can easily customize the processing parameters. For example, if the user want to do "quantile" normalization instead of "rsn" normalization, the user can run the following code. For more details, please read the help document of \Rfunction{lumiExpresso} function. 
...
In this case, when read the BeadStudio output file, we can ignore the "detection" and "beadNum" related columns. The function \Rfunction{lumiR} provides a parameter called "columnNameGrepPattern". we can set the string grep pattern of "detection" and "beadNum"  as NA. 

Here is some example code:
\begin{Sinput}
## load the data with empty detection and beadNum slots
>  x.lumi <- lumiR("fileName.txt", columnNameGrepPattern=list(detection=NA, beadNum=NA))
\end{Sinput}

Another good news is that the \Rfunction{vst} and \Rfunction{rsn} functions in the \Rpackage{lumi} package can sequentially process the data and handle such large data set.  

The solution can be like this:

1. Read the data file by smaller batches (e.g. 10 or just one by one), and then do the variance stabilization, i.e., \Rfunction{lumiT} function, for each data batch. 

2. Pick one sample as the target array for normalization and then using "RSN" normalization method. 
...
<<echo=F>>=
## load lumi annotation package
lib <- 'lumiHumanV1'		# Huamn lumi annotation package
if(require(GO) & require(annotate) & require(lib, character.only=TRUE)) {
	GOId <- 'GO:0004816' 		 # asparagine-tRNA ligase activity
	probe <- lookUp(GOId, lib, 'GO2ALLPROBES')
	# probes under 'GO:0004816'  category
...
As the annotation packages include the mappings between TargetID to nuID and ProbeID to nuID. We can easily map the targetID (or Probe Id) to nuID. The function can automatically check whether targetID or Probe Id  was used in the text data file, and convert them as nuID. Function \Rfunction{addNuId2lumi} can transfer a TargetID or Probe Id indexed  {\bf LumiBatch} object as an nuID indexed {\bf LumiBatch} object. And the mapping between the nuID and TargetID is kept in the featureData of the {\bf LumiBatch} object. If a {\bf LumiBatch} object has already been nuID indexed, the function will do nothing.
<<quiet=T>>=
...
The {\bf LumiBatch} object can also be directly transferred as nuID indexed at the very beginning of inputting data using \Rfunction{lumiR}. For example:
\begin{Sinput}
> ## load the data
...
if (require(GOstats) & require(lumiHumanV1) & require(xtable)) {
